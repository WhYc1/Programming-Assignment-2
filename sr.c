...
void A_start_physical_timer() {
    ...
    for (int j = 0; j < WINDOWSIZE; j++) { // Iterate physical slots
...
void A_input(struct pkt packet) {
    ...
    if (!IsCorrupted(packet)) {
        ...
        if (windowcount > 0) {
            ...
            int offset = -1; // Offset from windowfirst
            ...
            for (i = 0; i < windowcount; ++i) {
                int current_idx = (windowfirst + i) % WINDOWSIZE;
                if (buffer[current_idx].seqnum == packet.acknum) {
                    ack_in_window = true;
                    offset = i;           // Found offset
...
void A_timerinterrupt(void) {
    ...
    for (int k = 0; k < WINDOWSIZE; k++) {
        int idx = (windowfirst + k) % WINDOWSIZE;
...
void B_input(struct pkt packet) {
    ...
    int rcv_base = expectedseqnum;
    int seq_max_in_window = (rcv_base + WINDOWSIZE - 1) % SEQSPACE;          // Inclusive
    int seq_min_past_window = (rcv_base - WINDOWSIZE + SEQSPACE) % SEQSPACE; // Inclusive
    int seq_max_past_window = (rcv_base - 1 + SEQSPACE) % SEQSPACE;          // Inclusive
    ...
    if (!IsCorrupted(packet)) {
        if (in_receive_window) {
            ...
            int off = (packet.seqnum - rcv_base + SEQSPACE) % SEQSPACE;
